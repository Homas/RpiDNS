...
- New **Research** page (tab between RPZ log and Admin, deep-linkable as `#i2r/3/{unique|sql|tools}`) with three sections:
 ...
 - **Tools** - one target and one **Analyze** action that runs every applicable tool and shows the results side by side: RDAP/WHOIS, DNS records, NS/MX, reverse DNS (PTR), GeoIP, ASN, ping, traceroute, TLS certificate, reputation/threat intel, and a website preview.
...
- Results with a structured upstream are rendered readably with a per-card switch to the original: RDAP/WHOIS, reputation, GeoIP and ASN as summaries over their raw JSON, and the DNS tools (DNS records, NS/MX, PTR) as a record list over dig's full output.
- DNS records queries A, AAAA, HTTPS and TXT rather than A alone, and is offered for domain targets only - reverse DNS is the tool for an address.
...
- [CB] The web image installs the standalone `traceroute` (its default UDP mode needs no privileges) instead of relying on the busybox applet, which probes with a raw socket and cannot run as the unprivileged `www-data` user. The binary is also capability-marked (`cap_net_raw+ep`) for the privileged `-I`/`-T` modes, and the web container requests `NET_RAW` alongside `NET_ADMIN` so that marking can take effect. **Rebuild the web image and recreate the container** after upgrading, otherwise traceroute fails with `socket(AF_INET,3,1): Operation not permitted`.
