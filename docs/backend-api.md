...
- **Per-tool input class:** IP-only tools (`reverse_dns`, `geoip`, `asn`) require a valid IP address; domain-only tools (`dig`, `nsmx`, `tls_cert`, `reputation`, `website_preview`) require a valid domain; the remaining tools (`rdap`, `ping`, `traceroute`) accept a valid domain or IP. `dig` is domain-only because A/AAAA/HTTPS/TXT for an address is never meaningful — `reverse_dns` is the tool for an address.
- **`dig` record types:** with no `record_type` the tool queries `A`, `AAAA`, `HTTPS` and `TXT`, one dig per type, and returns the combined result. A single type can still be requested, which is what internal callers such as `ResearchResolver` use. The HTTPS RR (RFC 9460) is asked for as `TYPE65`: dig only understands the `HTTPS` mnemonic from BIND 9.16 onward, and older builds parse it as a hostname, silently turning the query into an A lookup for a name called "HTTPS" and reporting that name's NXDOMAIN as the target's. A dig that knows the type still prints the answer in `HTTPS` presentation format, and the numeric form is mapped back to `HTTPS` for display. Validation uses `InputValidator`. The frontend classifies the target the same way and only submits applicable tools.
- **RDAP target reduction:** RDAP/WHOIS is published per registered domain, so `rdap` looks up the *registrable domain* of the target rather than the hostname — `test.example.com` is queried as `example.com`. The boundary comes from the Public Suffix List via `PublicSuffix`, because it cannot be derived by counting labels: `bbc.co.uk` is a registration while `co.uk` behaves as a TLD, so `www.bbc.co.uk` is queried as `bbc.co.uk`. Only the list's ICANN section is used, so `someproject.github.io` reduces to `github.io` (the actual registration) rather than being treated as registrable itself. An IP target, a name that is already registrable, and a name that is itself a public suffix (`co.uk`) are passed through unchanged; an unrecognized suffix falls back to treating the last label as the public suffix. When a substitution happens the response prepends one line stating it, so a result is never mistaken for the hostname asked about. The bundled list is refreshed with `scripts/update_psl.php`.
...
For the tools with a structured upstream, `ResearchFormatter` puts a readable rendering in `output` and the original in `raw`, so a client can offer a two-view switch without a second request:

| Tools | `output` | `raw` |
|---|---|---|
| `geoip`, `asn`, `rdap`, `reputation` | Key/value or record summary | Pretty-printed JSON |
| `dig`, `nsmx`, `reverse_dns` | Record list — `TYPE  TTL  VALUE`, in the order dig returned them | dig's full output (header, flags, authority section) |

`raw` is `null` whenever there is no distinct second view: for tools with a single form (`ping`, `traceroute`, `tls_cert`), for output that is not valid JSON or not a dig response (a curl error or a failure to start is passed through untouched), and for JSON that has no summary renderer and is therefore already shown as pretty JSON. It is capped at 256 KiB, since a pretty-printed OTX report with hundreds of pulses travels in the same response.

The dig record list is built per query, not per response, because these tools issue several queries: answers come from ANSWER sections only (authority and additional records are glue, not answers), a queried type that returned nothing is named explicitly since "no AAAA" is itself a finding, and a response code other than `NOERROR` is attributed to the query it came from — `No records: AAAA, HTTPS (NXDOMAIN)` — rather than to the result as a whole.
