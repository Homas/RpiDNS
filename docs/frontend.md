...
- **SqlQueryTool** — an interface to run administrator-supplied read-only `SELECT` statements against the RpiDNS SQLite database, rendering the returned rows in a table with row count, truncation indication, and a CSV copy control. Fetches available table names to help build queries; clicking one inserts `select * from <table>` into an empty editor, or appends just the name when the editor already has content. **API endpoints:** `research_tables` (GET, table list) and `research_sql` (POST, query execution).
- **ResearchTools** — the tools panel: a single sticky command bar (target input, target-class badge, one **Analyze** button, and an Options toggle) above a responsive grid of result cards, one per tool. Analyze runs every selected tool that applies to the current target with a bounded fan-out (3 concurrent requests, fast tools first) so several results can be read side by side. The target is classified as domain or IP client-side with the same rules as the backend validators, so only applicable tools are rendered at all: a domain target omits the IP-only cards (reverse DNS, GeoIP, ASN) and an IP target omits the domain-only ones, with a single footnote naming what was left out instead of cards reporting a validation error. The Options drawer offers the same applicable set and holds the per-tool checkboxes and the DNS resolver. The resolver field applies to every tool, is seeded from `$research_dns_server` (Admin > Settings) via `research_config`, can be overridden for the panel, and carries a reset control that restores the configured default; a hint below it states whether the current value is inherited, an override, or the appliance resolver. Runs wait for `research_config` to load, so a tool launched straight from a context menu still inherits the default rather than racing the request. Each card carries quiet icon-only re-run and copy-output controls, and the website-preview card renders the returned PNG. Cards whose tool has a structured upstream show a readable rendering with a switch to the original: the JSON-backed tools (RDAP/WHOIS, reputation, GeoIP, ASN) a summary over their raw JSON, and the dig-based tools (DNS records, NS/MX, PTR) a record list over dig's full output. The switch tooltip names what the second view holds, from the tool definition's `rawLabel`. Both forms arrive in the same response, the switch appears only where both exist, copy follows whichever view is on screen, and the choice survives a re-run of that card. Tool definitions come from the shared `useNetworkTools` composable. Typing a new target resets all previously shown results. Exposes `runWith(target, tool)` — with an empty `tool` meaning "run everything applicable" — so it can be prefilled and auto-run from the `ToolsModal`. **API endpoint:** `research_tool` (POST).
...
- Research tools DNS resolver (the default the Tools panel inherits; blank = appliance resolver)
...
|--------|------|-------------|
| `RESEARCH_TOOLS` | `Array<{ name, label, icon, accepts, render, slow?, rawLabel? }>` | Constant array of tool definitions: endpoint tool name, human label, icon, accepted target class (`both`/`domain`/`ip`), result rendering kind (`text`/`image`), whether the tool is slow (run last), and what its second view holds (`raw JSON` / `raw dig output`) for tools the backend returns two forms for |
| `DNS_AWARE_TOOLS` | `Array<string>` | Tool names that honor the selected DNS resolver — every tool, derived from `RESEARCH_TOOLS`. The backend applies it three ways: `@server` for the dig-based tools, target pre-resolution for `ping`/`traceroute`/`tls_cert`/`website_preview`, and API-host pinning for `rdap`/`geoip`/`asn`/`reputation` |
